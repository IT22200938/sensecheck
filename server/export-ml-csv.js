...
async function buildSessionRow(session) {
  ...
  const allAttempts = await MotorAttemptBucket.getSessionAttempts(session.sessionId);
...
async function exportMLCSV() {
  try {
    ...
    for (const session of sessions) {
      ...
      const totalAttempts = attemptBuckets.reduce((sum, b) => sum + (b.count || 0), 0);
      
      if (totalAttempts > 0) {
        sessionsWithMotorData.push(session);
      }
    }
